...
import (
	...
	"errors"
	"fmt"
	"github.com/chubaodb/chubaodb/master/utils/apd"
	"github.com/chubaodb/chubaodb/master/utils/duration"
	"math"
	"reflect"
	"strconv"
	"time"
...
func getVarintLen(b []byte) (int, error) {
	...
	if length > len(b) {
		return 0, errors.New(fmt.Sprintf("varint length %d exceeds slice length %d", length, len(b)))
	}
	return length, nil
}
...
func DecodeVarintAscending(b []byte) ([]byte, int64, error) {
	...
	if length < 0 {
		...
		remB := b[1:]
		if len(remB) < length {
			return nil, 0, fmt.Errorf("insufficient bytes to decode uvarint value: %q", remB)
		}
		var v int64
...
func getBytesLength(b []byte, e escapes) (int, error) {
	...
	for {
		...
		if i == -1 {
			return 0, fmt.Errorf("did not find terminator %#x in buffer %#x", e.escape, b)
		}
		if i+1 >= len(b) {
			return 0, fmt.Errorf("malformed escape in buffer %#x", b)
		}
		skipped += i + 2
...
func decodeTime(b []byte) (r []byte, sec int64, nsec int64, err error) {
	if PeekType(b) != Time {
		return nil, 0, 0, errors.New("did not find marker")
	}
	b = b[1:]
...
func DecodeDurationAscending(b []byte) ([]byte, duration.Duration, error) {
	if PeekType(b) != Duration {
		return nil, duration.Duration{}, fmt.Errorf("did not find marker %x", b)
	}
	b = b[1:]
...
func DecodeDurationDescending(b []byte) ([]byte, duration.Duration, error) {
	if PeekType(b) != Duration {
		return nil, duration.Duration{}, errors.New("did not find marker")
	}
	b = b[1:]
...
func PeekLength(b []byte) (int, error) {
	...
	case floatNeg, floatPos:
		...
		if len(b) < 9 {
			return 0, fmt.Errorf("slice too short for float (%d)", len(b))
		}
		return 9, nil
	}
	...
	return 0, fmt.Errorf("unknown tag %d", m)
}
...
func decodeValueTypeAssert(b []byte, expected Type) ([]byte, error) {
	...
	b = b[dataOffset:]
	if typ != expected {
		return b, fmt.Errorf("value type is not %s: %s", expected, typ)
	}
	return b, nil
}
...
func PrettyPrintValueEncoded(b []byte) ([]byte, string, error) {
	...
	case Duration:
		...
		return b, d.String(), nil
	default:
		return b, "", fmt.Errorf("unknown type %s", typ)
	}
}
