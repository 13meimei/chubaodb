...
func decodeDecimal(buf []byte, tmp []byte, invert bool) ([]byte, *apd.Decimal, error) {
	...
	case decimalNaN, decimalNaNDesc:
		return nil, nil, fmt.Errorf("decimal does not support NaN values: %q", buf)
	case decimalInfinity, decimalNegativeInfinity:
		return nil, nil, fmt.Errorf("decimal does not support infinite values: %q", buf)
	case decimalZero:
		return buf[1:], apd.New(0, 0), nil
	}
	...
	case buf[0] == decimalPosSmall:
		...
		return r, makeDecimalFromMandE(invert, e, m, tmp2), nil
	default:
		return nil, nil, fmt.Errorf("unknown prefix of the encoded byte slice: %q", buf)
	}
}
...
func findDecimalTerminator(buf []byte) (int, error) {
	...
	return -1, errors.New(fmt.Sprintf("did not find terminator %#x in buffer %#x", decimalTerminator, buf))
}
...
func DecodeNonsortingDecimal(buf []byte, tmp []byte) (*apd.Decimal, error) {
	...
	case buf[0] == decimalPosLarge:
		...
		return dec, nil
	default:
		return nil, fmt.Errorf("unknown prefix of the encoded byte slice: %q", buf)
	}
}
