...
TEST_F(SelectFlowTest, SelectFlow_selection_table_read_mod)
{
    std::string start_key;
    std::string end_key;
    ...
    // test 1
    {
        ...
        // selection: mod(id,2) = 1
        dspb::Expr expr_equ;
        int mod_val = 2;
        int eqe_val = 1;
        expr_equ.set_expr_type(dspb::Equal);

        auto expr_mod = expr_equ.add_child();
        expr_mod->set_expr_type(dspb::Mod);

        auto expr_mod_int_column = expr_mod->add_child();
        expr_mod_int_column->set_expr_type(dspb::Column);
        expr_mod_int_column->mutable_column()->CopyFrom(ptr_table_account->GetColumnInfo("id"));
        auto expr_mod_int_value = expr_mod->add_child();
        expr_mod_int_value->set_expr_type(dspb::Const_Int);
        expr_mod_int_value->set_value(std::to_string(mod_val));

        auto expr_equal_int_value = expr_equ.add_child();
        expr_equal_int_value->set_expr_type(dspb::Const_Int);
        expr_equal_int_value->set_value(std::to_string(eqe_val));

        selection->add_filter()->CopyFrom(expr_equ);
    ...
    // test 2
    {
        ...
        int mod_val = 2;
        int eqe_val = 0;
        expr_equ.set_expr_type(dspb::NotEqual);
