...
const static uint32_t default_row_num  = 1000;
...
void genKvPrimaryKeyData(std::map<std::string, std::string > & mp)
{
    ...
    for (auto id = 0u; id < default_row_num ; id++) {
        std::string key;
        std::string value;

        // make row data
        std::map<col_id_type, std::string> row;
        row.emplace(1, std::to_string(1000000+id));
        row.emplace(2, "name_" + std::to_string(1000000+id));
        row.emplace(3, std::to_string(1000000+id));
...
void genKvPrimaryKeyData2(std::map<std::string, std::string > & mp)
{
    typedef int col_id_type;
    for (auto id = default_row_num; id < 2*default_row_num ; id++) {
        std::string key;
        std::string value;
...
TEST_F(SelectFlowTest, SelectFlow_data_sample) {
    ...
    range_server_->ranges_.Find(1)->ResetStatisSize();
    ...
    {   // test SelectFlow data_sample test1
        ...
        auto data_sample_processor = select_flow->add_processors();
        data_sample_processor->set_type(dspb::DATA_SAMPLE_TYPE );
        auto data_sample = data_sample_processor->mutable_data_sample();

        data_sample->set_method(dspb::DataSample_SampleMethod_RESERVIOR_SAMPLING );
        for ( const auto & col : ptr_table_account->GetAllColumns()) {

            data_sample->add_columns()->CopyFrom(ptr_table_account->GetColumnInfo(col.id()));
        }
        data_sample->mutable_range()->set_start_key(start_key);
        data_sample->mutable_range()->set_end_key(end_key);
        data_sample->set_ratio(0.0);
        ...
        ASSERT_TRUE(resp.select_flow().rows_size() == default_row_num * 0 ) << resp.select_flow().rows_size();
    ...
    {   // test SelectFlow data_sample test1
        ...
        ASSERT_FALSE(resp.header().has_error());
        ASSERT_TRUE(resp.has_select_flow());
        ASSERT_TRUE(resp.select_flow().rows_size() == default_row_num * 0.1 ) << resp.select_flow().rows_size();
    ...
    {   // test SelectFlow data_sample test1
        ...
        ASSERT_FALSE(resp.header().has_error());
        ASSERT_TRUE(resp.has_select_flow());
        ASSERT_TRUE(resp.select_flow().rows_size() == default_row_num * 1.0 ) << resp.select_flow().rows_size();
    ...
    {   // test SelectFlow data_sample test1
        ...
        ASSERT_FALSE(resp.header().has_error());
        ASSERT_TRUE(resp.has_select_flow());
        ASSERT_TRUE(resp.select_flow().rows_size() == 2*default_row_num * 0.0 ) << resp.select_flow().rows_size();
    ...
    {   // test SelectFlow data_sample test1
        ...
        ASSERT_FALSE(resp.header().has_error());
        ASSERT_TRUE(resp.has_select_flow());
        ASSERT_TRUE(resp.select_flow().rows_size() == 2*default_row_num * 0.1 ) << resp.select_flow().rows_size();
    ...
    {   // test SelectFlow data_sample test1
        ...
        ASSERT_FALSE(resp.header().has_error());
        ASSERT_TRUE(resp.has_select_flow());
        ASSERT_TRUE(resp.select_flow().rows_size() == 2*default_row_num * 1.0 ) << resp.select_flow().rows_size();
